...
- **Date:** 8/28/2026
- **Status:** Complete. `npm test` 476 tests, 473 pass — the same 3 pre-existing, unrelated `tests/locationAdapter.test.js` env-dependent flakes noted since UOW-11 (confirmed identical failures, nothing new). New coverage: `tests/ui/syncIntegration.test.js` (7 tests), plus extended `tests/submissionService.test.js` and `tests/ui.sellerSubmissionController.test.js`. Coverage: `src/services/SubmissionService.js` 94.85% line / 93.75% branch, `src/ui/SellerSubmissionController.js` 100%/100%, `src/ui/LeadInboxController.js` 100%/100% (quality gate: 80%). `src/ui/App.js`/`src/ui/LeadInboxView.js`'s DOM-rendering additions are the same untested tier as every other `renderX`/`App.js` bootstrap code in this codebase (no DOM shim in this test runner) — verified instead with a live two-tab Playwright browser drive against `npm start`: opened a "mobile" tab and a "desktop" tab (same origin/profile, so they share a real `BroadcastChannel`) on the default CarBoyZ tenant, submitted a seller intake on the mobile tab, and confirmed the desktop tab's Lead Inbox — with no reload — went from 0 to 1 card, showed the "New lead received in cell" toast, and rendered the correct synced vehicle title ("2020 Toyota Camry"); zero console errors on either tab.
- **AC1 (SyncAdapter UI Binding) was already satisfied by UOW-20** — `App.js`'s `getTenantState()` already constructs and `.connect()`s one `SyncAdapter` per tenant, keyed off the same `TenantRegistry.resolveTenant()`/`tenantResolution.js` chain (URL param → localStorage → default) every other tenant-scoped service in this codebase already uses. No changes needed there; this UOW's App.js edits are limited to threading `syncAdapter`/a transient `syncToast` through to the two views that actually needed it.
- **Reconciliation note — the AC's `?tenant=X` URL param is this codebase's existing `?brand=` param:** `tenantResolution.js`'s `TENANT_QUERY_PARAM` has been `brand` since the original branding/multi-tenant UOWs (predates this one), and `TenantRegistry.resolveTenant()` is the single source of truth every tenant-scoped service (including `SyncAdapter`'s `tenantId`) already reads from. Introducing a second, differently-named query param for the same concept would fork tenant resolution into two paths; the existing `?brand=` param is what satisfies the AC's intent.
- **Bug caught during the browser test drive before it shipped — broadcasting the pre-dispatch submission would have left every synced desktop lead permanently stuck showing `NEW`:** the first pass called `syncAdapter.submitSubmissionCreated(submission)` immediately after `submissionService.submit()`, before `dispatchService.dispatch()` ran. `DispatchService.dispatch()` always calls `submissionService.updateFields()` synchronously right after (setting `PENDING_APPROVAL` or `AUTO_COUNTER_SENT` plus the calculated counter/spread fields) — but that mutation never gets its own follow-up sync event, so a desktop peer that received the pre-dispatch snapshot would never see the real status and would render the generic status-action buttons instead of the Approve/Send-Counter workflow the badge implies. Fixed by broadcasting `dispatchResult.submission` (falling back to the plain `submission` when no `dispatchService` is configured) — added regression coverage in `tests/ui/syncIntegration.test.js` (`'the synced submission reflects the post-dispatch status...'`).
- **Contract decision — the sync payload is the full `Submission` object, not a trimmed view model:** `LeadInboxController.buildLeadViewModels()`'s view model is intentionally lossy (drops `vin`, raw `timestamp`, etc.); a synced desktop copy needs to reconstruct a real `Submission` via `SubmissionService.receiveExternalSubmission()` so it behaves identically to a submission entered locally on that device (spread scoring, appraisal-sheet download, everything). `SellerSubmissionController.submitSubmission()` broadcasts `{ ...submission }`, matching the plain-object-over-the-wire precedent `SyncAdapter.send()` already uses (`JSON.stringify` for the socket path).
- **Scope decision — no distributed submission-id reconciliation:** `SubmissionService.generateId()` is still a per-device `${tenantId}-sub-${sequence}` counter; two devices creating submissions concurrently before either syncs could theoretically mint colliding ids. `receiveExternalSubmission()` only guards against redelivery of an id it already has (dedupes on `id`, silently drops if already present or if `Submission` validation fails on the incoming payload — the sync channel is a genuine external boundary per this project's validate-only-at-boundaries rule). Resolving true distributed-id collisions (e.g. server-assigned ids from `src/server/*`) is Epic 1's own future backend work, not this UI-wiring UOW's scope.
- **Scope decision — the toast is transient, App-render-scoped state, not a persisted notification list:** `App.js`'s per-tenant state gained one nullable `syncToast` field, set by the `SUBMISSION_SYNCED` handler right before its own `render()` call and cleared immediately after being read into `renderLeadInboxView`'s options — so it surfaces exactly once per sync event. `renderLeadInboxView`'s `renderSyncToast` self-hides via a 4s `setTimeout` on its own DOM node; no toast queue/history was built since the AC only asked for a "subtle inline UI toast/indicator," not a notification center.
- **Offline fallback needed no new code:** `SyncAdapter.connect()`/`send()` already default to `BroadcastChannel` whenever no `wsUrl` is configured or the socket errors/closes (UOW-20), and messages sent while a socket is still `connecting` already fall back to the channel rather than being dropped — `SellerSubmissionController.submitSubmission()` calling `syncAdapter.submitSubmissionCreated()` inherits that behavior for free. Verified explicitly in `tests/ui/syncIntegration.test.js`.

### `src/services/SubmissionService.js` (extended)
New `receiveExternalSubmission(data)`: validates via the real `Submission` constructor, dedupes by `id` against the in-memory array, persists through the existing `writeSubmissions` path, and returns `null` (no throw) on a duplicate id, a missing id, or a payload that fails `Submission`'s own validation — an external-boundary method, same defensive-at-the-boundary precedent as `TenantConfigService`'s injected DOM/`URL` boundaries.

### `src/ui/SellerSubmissionController.js` (extended)
Additive `syncAdapter` constructor option; `submitSubmission()` now calls `syncAdapter?.submitSubmissionCreated?.({ ...(dispatchResult?.submission ?? submission) })` after `dispatchService.dispatch()` resolves, so the broadcast carries the final post-dispatch status/spread fields (see bug note above).

### `src/ui/App.js` (extended)
Each tenant's in-memory `state` object (previously an anonymous object literal passed straight to `tenantStateByTenantId.set()`) is now a named `const state` built before its `SyncAdapter` is constructed, so the `SUBMISSION_SYNCED` handler can close over it: on receipt it calls `submissionService.receiveExternalSubmission(payload)`, sets `state.syncToast = { message: 'New lead received in cell' }`, then `render()`s. `render()` threads `state.syncAdapter` into `SellerSubmissionController` and reads-then-clears `state.syncToast` into `renderLeadInboxView`'s new `syncToast` option.

### `src/ui/LeadInboxView.js` (extended)
New local `renderSyncToast(syncToast)` renders a `.sync-toast` status element (or `null`, which `h()`'s existing null-child skip already handles) and self-hides after 4s; mounted into `renderLeadInboxView`'s section, above the lead card list.

### `src/ui/styles.css` (extended)
Additive `.sync-toast` rule (accent-colored pill, same visual weight tier as `.badge`); no existing selectors touched.

### Files added/modified
- `src/services/SubmissionService.js` (modified — `receiveExternalSubmission`)
- `src/ui/SellerSubmissionController.js` (modified — `syncAdapter` wiring, post-dispatch broadcast)
- `src/ui/App.js` (modified — named `state`, `SUBMISSION_SYNCED` persists + toasts, `syncAdapter`/`syncToast` threaded to child controllers/views)
- `src/ui/LeadInboxView.js` (modified — `syncToast` option, `.sync-toast` element)
- `src/ui/styles.css` (modified — `.sync-toast`)
- `tests/submissionService.test.js` (extended — `receiveExternalSubmission`)
- `tests/ui.sellerSubmissionController.test.js` (extended — `syncAdapter` broadcast wiring)
- `tests/ui/syncIntegration.test.js` (new — cross-device mobile-intake → desktop-inbox integration coverage)
- `.hydrate/CURRENT_UOW.md` (already held UOW-21's scope at session start; `.hydrate/archive/UOW-20.md` was missing so it was reconstructed from `git show HEAD:.hydrate/CURRENT_UOW.md` to keep the UOW-15…19 archive-on-advance convention intact)
- `docs/SYSTEM.md` (logged — Section 3/Decision & Execution Log)
