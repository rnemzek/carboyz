...
## [UOW-09] — Dynamic Geocoding Search & H3 Cell Hydration (Google Places)
- **Date:** 2026-08-20
- **Status:** Complete. Cross-repo, on top of the Spatial Core / Domain Overlay Contract refactor. `spatial-core` tests: 41/41 passing. `carboyz` tests: 185/185 passing, `locationAdapter.js` at 100.00% line / 80.49% branch coverage.
- **Scope:** Replace UOW-08's fully-static ZIP/city gazetteer with a real geocoding fallback (Google Places Text Search), and wire spatial-core's previously library-only "Look Far" gap-fill pipeline (`GooglePlacesAdapter`/`SpatialCellIndex`/`discoverNearby`, added in the prior refactor but never called from carboyz) into `MapView.js` so map movement and location search dynamically hydrate dealership pins instead of relying on seed data.
- **Note:** spatial-core's prior refactor commit (`e989da1`) had a second, uncommitted local diff sitting in its working tree — `SpatialCellIndex.markCellHydrated`/`isCellFresh` (TTL-based freshness) and `GooglePlacesAdapter` place-metadata passthrough, already described in the journal entry above but never actually committed in the spatial-core repo. Committed that first, standalone (`d1649f5`), before starting this UOW's work.

### Tasks completed
- **9.1 (spatial-core, geocoding):** Added `GooglePlacesGeocoder` to `src/geocoder.ts`, implementing the existing `GeocoderResolver` interface via Places API (New) Text Search (`places:searchText`, field mask `places.location,places.displayName,places.formattedAddress,places.viewport`). Returns `GeocodedPlace` (`{ lat, lng, displayName, formattedAddress, boundingBox? }`, `boundingBox` derived from the response's `viewport` low/high corners). Same injectable `apiKey`/`fetchImpl` + no-key-short-circuits-to-null pattern as `GooglePlacesAdapter`. Exported `GooglePlacesGeocoder`, `GeocodedPlace`, `GeocodedBoundingBox` from `index.ts`. New `tests/geocoder.test.ts` (10 tests): no-resolver/blank-input defaults, no-key short circuit, header/body shape, successful mapping incl. `boundingBox`, missing-viewport, non-ok response, unparseable body, zero results.
- **9.2 (carboyz, search resolution):** `locationAdapter.js`'s `resolveLocationQuery` is now `async` and two-tier: the offline gazetteer stays the fast/deterministic first pass (unchanged matching logic), and anything it doesn't recognize — a street address, or a city/ZIP outside the 7-entry demo list (e.g. "Pensacola, FL", "32501") — falls through to an injected `geocoder` (default: `createDynamicGeocoder()`, a `GooglePlacesGeocoder` reading `GOOGLE_PLACES_API_KEY` via the same `options.apiKey` → `process.env` → `window.CARBOYZ_*` pluggable-fallback shape `chatFilterAdapter.js` uses for its Anthropic key). `resolveGooglePlacesApiKey` exported for reuse by `MapView.js`'s Places gap-fill adapter, so both features share one configured key.
- **9.3 (carboyz, search UI):** `MapView.js`'s location-search modal submit handler is now `async`/awaits `resolveLocationQuery`, with a "Searching…"/disabled button state and a distinct network-failure error message (existing "couldn't find X" copy stays for a genuine no-match). `applyUserLocation` (search submit, 🎯 locate-me, initial fallback) is unchanged in shape — still `map.flyTo` + `filterDealersNearby` + re-render — but now also triggers hydration (9.4).
- **9.4 (H3 cell hydration + "Look Far" gap-fill trigger):** `MapView.js` now constructs one `SpatialCellIndex` (6h TTL) and one `GooglePlacesAdapter` (`includedType: 'car_dealer'`, the real Places API New type — "auto_dealer" isn't one) per mount, plus a `discoveredDealers` pool merged with seeded `dealers` for rendering/filtering (`allDealers()`). `hydrateVisibleArea(center)` derives 1–3 representative points (map center + viewport NE/SW corners via `map.getBounds()`), dedupes by H3 cell (`latLngToCell`), skips any `cellIndex.isCellFresh` cell, and calls `discoverNearby(point, 24km, { adapter, index, includedType: 'car_dealer' })` for the rest — a per-representative-point call rather than one call per fine H3 cell (res-8 hexagons are ~0.46 km², far smaller than a Places search radius; iterating every visible cell would fire redundant/rate-limited calls for no benefit). Newly-discovered features merge into `discoveredDealers`, `nearbyDealers` recomputes, and the layer re-renders. Wired from three triggers: `map.on('moveend'/'zoomend')` (debounced 600ms via `scheduleHydration`), and directly from `applyUserLocation` (so a search/locate-me doesn't wait for a subsequent pan). Places/network failures per-point are caught and swallowed (cell stays un-hydrated for a later retry) rather than surfacing to the user.
- **9.5 (tests):** Rewrote `tests/locationAdapter.test.js` for the async signature (185 total tests, +13 net over UOW-08's baseline incl. some consolidation): gazetteer hits still resolve without touching the geocoder; a fake injected `geocoder` proves the Pensacola/32501-style fallback path (mapping `displayName`/`formattedAddress`/`boundingBox` through); `createDynamicGeocoder` with no key configured resolves to `null` without a network call. No new unit-test target for the `MapView.js` hydration logic itself — same rationale as UOW-08 (DOM + `maplibregl` global required, kept out of the unit-test-covered adapter layer); validated instead via the Playwright pass below.
- **9.6 (quality gate / Playwright):** `npm test` green in both repos. Ran a headless Playwright pass against the live app (`npx serve .`) with `window.fetch` mocked at the `places.googleapis.com` `searchText`/`searchNearby` URLs (no real `GOOGLE_PLACES_API_KEY` available in this environment) returning real Pensacola, FL coordinates (`30.4213, -87.2169`) and one fabricated "Pensacola Bay Auto Mall" dealer: Map tab → 3 seeded-dealer pins render near the Leland, NC fallback location (each triggering a `searchNearby` gap-fill call on mount, confirming the hydration trigger fires without a location search) → opened the search modal, submitted "Pensacola, FL" → one `searchText` call fired, location bar updated to "📍 Pensacola, FL (25 mi)" (label sourced entirely from the mocked geocoder response, not any hardcoded table), map flew to the resolved point → after the debounced moveend hydration settled, pin count dropped from 3 to 1 and the surviving pin's `aria-label` read "Pensacola Bay Auto Mall" — i.e. the NC-seeded dealers correctly filtered out of range and the dynamically-discovered dealer rendered in their place. Zero console errors throughout.

### Files added/modified
- spatial-core: `src/geocoder.ts`, `src/index.ts` (modified); `tests/geocoder.test.ts` (new)
- carboyz: `src/adapters/locationAdapter.js`, `src/ui/MapView.js` (modified); `tests/locationAdapter.test.js` (rewritten)
- `ROADMAP.md`, `.hydrate/CURRENT_UOW.md` (UOW-09 checked off / logged)

### Test output
```
spatial-core: 41 pass / 0 fail
carboyz:      185 pass / 0 fail
```
